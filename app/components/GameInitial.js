...
export default function GameInitial({ onBack }) {
  ...
  useEffect(() => {
    loadQuiz("quiz_initial", ID, 20).then(items => { setList(items); setReady(true); });
  }, []);
  ...
  if (phase === "end") return <Rslt score={score} maxScore={list.length * 20} onRetry={() => { setIdx(0); setScore(0); setHIdx(0); setInput(""); setGlow(null); setPhase("playing"); setTimeout(() => iref.current?.focus(), 100); }} onBack={onBack} />;
  const q = list[idx];
  return (
    <Wrap>
