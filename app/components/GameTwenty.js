...
export default function GameTwenty({ onBack }) {
  ...
  useEffect(() => {
    loadQuiz("quiz_twenty", TW_FB, 20).then(items => { setList(items); setReady(true); });
  }, []);
  ...
  const submit = () => {
    ...
    if(val===list[idx].word){
      ...
      setInput(""); // FIX: 즉시 초기화
      if(hStep<list[idx].hints.length-1){ setHStep(h=>h+1); sfx.reveal(); setTimeout(()=>iref.current?.focus(),100); }
      else { setHistory(h=>[...h,{word:list[idx].word,ok:false,pts:0,reason:'힌트 4개 모두 사용'}]); setGlow("wrong"); setShake(true); sfx.wrong(); setTimeout(()=>{setShake(false);setGlow(null);setTimeout(()=>iref.current?.focus(),100);},450); }
    }
  };
