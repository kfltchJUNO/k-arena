...
export function Wrap({ children }) {
  return <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#06090f,#0f172a)", display:"flex", flexDirection:"column", fontFamily:"system-ui,sans-serif", color:"#e2e8f0" }}><style>{KF}</style>{children}</div>;
}
...
export function TInput({ value, onChange, onEnter, placeholder="입력하세요", glow }) {
  const bc = glow==="correct"?"#22c55e":glow==="wrong"?"#ef4444":"rgba(255,255,255,0.11)";
  return <input value={value} onChange={onChange} onKeyDown={e=>{ if(e.key==="Enter"){e.preventDefault();onEnter&&onEnter();} }} placeholder={placeholder} autoComplete="off" style={{ flex:1, padding:"13px 16px", borderRadius:13, border:`1.5px solid ${bc}`, background:"rgba(255,255,255,0.05)", color:"#e2e8f0", fontSize:"1rem", fontFamily:"inherit", outline:"none", transition:"border-color .2s" }}/>;
}
                ...
                {detail.map((d,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:d.ok?"rgba(34,197,94,0.07)":"rgba(239,68,68,0.07)",border:`1px solid ${d.ok?"rgba(34,197,94,0.2)":"rgba(239,68,68,0.2)"}`,borderRadius:10,padding:"9px 13px",animation:`fadein .3s ease-out ${i*.04}s both`}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      ...
                      <span style={{color:"#e2e8f0",fontWeight:600,fontSize:"0.88rem"}}>{d.word}</span>
                      {d.answer&&!d.ok&&<span style={{color:"#64748b",fontSize:"0.74rem"}}>→ {d.answer}</span>}
                    </div>
